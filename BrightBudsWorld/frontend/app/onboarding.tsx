...
import { setAudioEnabled, speak, voice } from '@/src/utils/audio';
...
export default function Onboarding() {
  ...
  const next = () => {
    if (step === 0 && !avatarId) { voice.chooseAvatar(); return; }
    if (step < 3) {
      setStep(step + 1);
      // Play voice prompt for the next step
      setTimeout(() => {
        if (step + 1 === 1) voice.whatsYourName();
        else if (step + 1 === 2) voice.howOld();
      }, 300);
    }
    else {
      voice.letsPlay();
